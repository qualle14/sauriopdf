...
The Liberation Fonts is font collection which aims to provide document layout
compatibility as usage of Times New Roman, Arial, Courier New.
...
1. Get source
   ...
   - Or downloading the tar.gz file from
     [releases](https://github.com/fontforge/fontforge/releases).
...
3. Install to system
   ...
   Fedora Users :
   - One can manually install the fonts by copying the TTFs to `~/.fonts` for user wide
     usage
...
1. Before packaging a new release based on a new source tarball, you have to update the
   version suffix in the Makefile:
   ```
   VER = [NEW_VERSION]
   ```
